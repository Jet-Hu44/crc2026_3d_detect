...
class MainWindow(QMainWindow):
    ...
    def check_txt_files(self):
        ...
        if any(file.endswith('.txt') for file in os.listdir(directory)):
            print("已发送NEEPU-HS-R1.txt到裁判盒,识别结束,准备关闭软件界面。")
            self.close()
        else:
            print("结果文件未生成")
    ...
    def send_result(self, file_path):
        file_path = '/home/HwHiAiUser/Desktop/result_r/NEEPU-HS-R1.txt' # 目录名 #--------------------------------------------------------------------------------------------------比赛注意
    ...
    def process_detection_cycle(self):
        ...
        output_path = '/home/HwHiAiUser/ultralytics-main/runss/result/NEEPU-HS-R1.txt' # 目录名 #--------------------------------------------------------------------------------------------------比赛注意
        # 开始后2秒删除所有文件
        time.sleep(2)
    ...
    def start_detection(self):
        ...
        self.send_team_id(0, "幻视")  # 发送团队ID #--------------------------------------------------------------------------------------------------比赛调试：队伍ID
    ...
    def display_results(self, result_list):
        ...
        for result in result_list:
            ...
            if len(result) > 6 and result[6] is not None:
                ...
                """
                if 0.8 <= distance_m <= 1.8: # 桌面距离 #--------------------------------------------------------------------------------------------------比赛注意
                    # 红色字体
...
if __name__ == '__main__':
    """更换自己的pt权重文件"""
    detector = YoloOrbbecDetector(weights='best3.pt', device='0') # 权重替换 #--------------------------------------------------------------------------------------------------比赛注意
