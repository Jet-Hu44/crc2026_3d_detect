...
class YoloOrbbecDetector:
    def __init__(self, weights='yolov8n.pt', device='0', half=False):
        ...
        try:
            ...
            print(f"已加载YOLOv8模型,类别数:{len(self.model.names)}")
...
class MainWindow(QMainWindow):
    def __init__(self, detector):
        ...
        self.video_folder = "/home/HwHiAiUser/ultralytics-main/runss" # 目录名 #--------------------------------------------------------------------------------------------------比赛注意
        self.txt_output_folder = "/home/HwHiAiUser/ultralytics-main/runss/labels" # 目录名 #--------------------------------------------------------------------------------------------------比赛注意
    ...
    def initUI(self):
        ...
        self.status_label = QLabel("空闲", self)
    ...
    def check_txt_files(self):
        directory = '/home/HwHiAiUser/ultralytics-main/runss/result' # 目录名 #--------------------------------------------------------------------------------------------------比赛注意
    ...
    def send_result(self, file_path):
        file_path = '/home/HwHiAiUser/Desktop/result_r/xuexiao-tuanduiid-R1.txt' # 目录名 #--------------------------------------------------------------------------------------------------比赛注意
    ...
    def process_detection_cycle(self):
        folders = ['/home/HwHiAiUser/ultralytics-main/runss/d'] # 目录名 #--------------------------------------------------------------------------------------------------比赛注意
        output_path = '/home/HwHiAiUser/ultralytics-main/runss/result/xuexiao-tuanduiid-R1.txt' # 目录名 #--------------------------------------------------------------------------------------------------比赛注意
        # 开始后2秒删除所有文件
        time.sleep(2)
        self.delete_all_files_in_folder('/home/HwHiAiUser/ultralytics-main/runss/labels') # 目录名 #--------------------------------------------------------------------------------------------------比赛注意
        self.delete_all_files_in_folder('/home/HwHiAiUser/ultralytics-main/runss/d') # 目录名 #--------------------------------------------------------------------------------------------------比赛注意
        # 检测10秒
        time.sleep(10)
        self.move_txt_files('/home/HwHiAiUser/ultralytics-main/runss/labels', '/home/HwHiAiUser/ultralytics-main/runss/d') # 目录名 #--------------------------------------------------------------------------------------------------比赛注意
        # 等待0.5秒后处理结果
        time.sleep(0.5)
        total_counts = defaultdict(int)
        for folder in folders:
            """
            这里min_occurrences为计算所有帧稳定出现的次数，根据实际/home/kybot/yolov5/runss/d内的txt文件数量动态调整，
        ...
        self.delete_all_files_in_folder('/home/HwHiAiUser/Desktop/result_r') # 目录名 #--------------------------------------------------------------------------------------------------比赛注意
        time.sleep(0.5)
        self.copy_txt_files('/home/HwHiAiUser/ultralytics-main/runss/result', '/home/HwHiAiUser/Desktop/result_r') # 目录名 #--------------------------------------------------------------------------------------------------比赛注意
        file_path = '/home/HwHiAiUser/Desktop/result_r/xuexiao-tuanduiid-R1.txt' # 目录名 #--------------------------------------------------------------------------------------------------比赛注意
        self.send_file(1, file_path)
        self.s.close()
        print("关闭socket")
        time.sleep(5)
        self.delete_all_files_in_folder('/home/HwHiAiUser/ultralytics-main/runss/result') # 目录名 #--------------------------------------------------------------------------------------------------比赛注意

    # ============ 关键线程控制 ============
    def start_detection(self):
        self.status_label.setText("识别中") # 更新状态为“识别中”
        self.send_team_id(0, "tuanduiid")  # 发送团队ID #--------------------------------------------------------------------------------------------------比赛调试：队伍ID
...
if __name__ == '__main__':
    """更换自己的pt权重文件"""
    detector = YoloOrbbecDetector(weights='bbest.pt', device='0') # 权重替换 #--------------------------------------------------------------------------------------------------比赛注意
